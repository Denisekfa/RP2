turtles-own
  [ sick?                ;; se verdadeira, a turtle está infectada
    vacinado?            ;; esta imune a doença por causa de vacina
    imune?               ;; esta imune a doença porque já contraiu ela antes
    contagioso?					 ;; se verdadeiro, agente é contagioso. Começa 8 dias depois de se infectar e dura por + 8 dias
    sick-time            ;; tempo infectado em dias
    age
  ]                      ;; idade da turtle em dias
...
to setup-constants
  ...
  set carrying-capacity 1000                    ;; capacidade máxima
...
to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick? [ ifelse-value contagioso? [ red ] [ pink ] ] [ ifelse-value imune? [ grey ] [ ifelse-value vacinado? [ green ] [ white ]] ] ]
end
...
to infect ;; turtle procedure
  ask other turtles-here with [not contagioso? and not immune? and not vacinado?] ;;se não está contagioso e não imune e não vacinado
  ...
  if count turtles < carrying-capacity and random-float 100 < chance-reproduce
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy
        if random-float 100 < %immune [set vacinado? true]
      ] ]
end
...
"" ""
PENS
"Doentes" 1.0 0 -2674135 true "" "plot count turtles with [ sick? ]"
